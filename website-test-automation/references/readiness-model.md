...
- Every outcome or evidence report must contain a concrete observed result signal such as passed, failed, verified, observed, completed, succeeded, executed, ran, a positive pass/failure/error count, an exit code, or an HTTP status. Bare discovery totals such as `12 tests`, `12 tests selected`, `12 tests found`, or `12 tests dry-run only` are not proof. Zero counts, skipped/pending/disabled/blocked/ignored/collected-only counts, and not-run claims such as `0 tests passed`, `Tests 0 passed`, `0 browser projects passed`, `failed tests: 0`, `12 tests skipped`, `12 tests pending`, `tests not run`, and `never executed` are not proof either. Result verbs require at least three substantive context tokens, so bare `passed`, `verified`, `worked`, and `looks good` are invalid. The scorer checks a bounded punctuation-delimited context around each result-verb candidate and fails closed when an unpunctuated prefix exceeds that context; negated or unfinished phrases such as `not verified yet`, `never completed`, and `yet to be verified` are invalid, while a directly observed `failed` result remains valid evidence. Expectation language anywhere in the string makes result-count/status signals invalid unless an actual/observed marker appears after the expectation and before that numeric signal, or a later passed/failed/verified/observed/completed/succeeded result verb establishes the outcome: `expected 201; actual status 200` and `planned 12; 12 passed` are valid, while `HTTP status 200 expected; actual unknown` is not. When a placeholder is present, the concrete result must appear first, as in `12 tests passed; TODO follow-up`.
...
- Global limits are 10,000 scanned files, 10,000 directories, 50,000 directory entries, 32 MiB of scored text, and 20 manifest candidates. Directory traversal uses bounded iteration instead of materializing unbounded directory listings. Extra or unreadable content is reported in `resourceBudget.warnings` and prevents 90+.
...
- Concrete-result candidates are collected once, ordered once, and evaluated with bounded local windows so repetitive proof text stays within the same linear-scale resource model.
