...
- `version` is exactly `1`, and `projects` contains at least two entries with at least two unique project IDs. Duplicate IDs are invalid.
- Every project has nonblank, non-placeholder string values for `id`, `target`, `command`, and `outcome`, plus a nonempty string array in `evidence`. Values such as TODO, TBD, pending, not run, replace, placeholder, and example-only are invalid.
- Every item in `evidence` is a nonblank path relative to the scored root and resolves to an existing ordinary non-symlink file inside that root. The file contents must be nonempty and not placeholder-only. A placeholder word in a legitimate filename or within a report that already records a concrete result does not by itself invalidate the evidence. Absolute paths, traversal, symbolic links in the final path or parent chain, directories, oversized text files, and paths outside the root are invalid.
- The manifest itself is root-contained, valid JSON, within the text-size limit, and an ordinary non-symlink file whose parent chain contains no symbolic links.
