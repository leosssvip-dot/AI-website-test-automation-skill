...
1. Assign every case a case disposition: `automate-now`, `automate-later`, `browser-smoke`, `manual/live`, `exploratory`, `human-logic-risk`, or `risk-note/not-in-scope`.
...
| --- | --- | --- | --- |
| Response-only review | User asks for review, plan, coverage, recommendations, or test cases without implementation. | Build product model, run the Human Reasonableness Review Gate, detect mismatches, generate cases, run the Post-Test-Case Disposition Gate, and stop before file edits unless the user asks to implement. | Product model, logic findings ledger, mismatches, coverage matrix, source-backed cases, case disposition, evidence inspected, and next automation candidates. |
| Test-case authoring | User emphasizes test cases, PRD/source/design understanding, or QA package generation. | Read requirements/code/design sources, run the Human Reasonableness Review Gate, write cases with source status, then classify every case through the disposition gate. | Prioritized P0/P1/P2 cases, source evidence, human expectation risks, assumptions, unknowns, coverage gaps, and automation recommendation per case. |
