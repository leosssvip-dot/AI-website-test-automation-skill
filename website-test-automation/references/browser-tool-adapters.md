...
Apply the canonical [Browser Evidence Condition](scenario-workflows.md#browser-evidence-condition) before choosing an adapter. When it applies, use the lowest-cost browser tool that proves the acceptance signal; otherwise do not start a browser or manufacture a scoped-skip reason. Use `assets/checklists/browser-smoke-evidence.md` when the required evidence has more than one browser observation.
